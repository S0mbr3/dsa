export default class ArrayList {
	...
	if (Math.floor(this.tail * this.bytes) >= this.capacity) {
	    ...
	    let i = gap;
	    for (let j = 0; j < this.tail - this.head; ++i, ++j) {
		view[i] = this.view[this.head+j]
		//console.log(`i:${i} j:${j} view:${view[i]} this.view:${this.view[this.head+j]} tail:${this.tail}`);
	    }
	    this.buf = buf;
	    this.view = view;
	    this.head = gap;
